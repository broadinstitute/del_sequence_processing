...
def WTMrev(f): #This function analyzes each read and identifies the library member it corresponds to
    ...
    if f.find("CCCTG",0) == -1:
      old_chars = "ACGT"
      replace_chars = "TGCA" #since the read can correspond to either template strand or complementary strand, this converts everything to the template strand
   ...
   # defining codons. Note, that constant regions are ignored in order to minimize losing reads due to mutations therein
    ...
    # this is a unique number assigned to each an every library member, from 1 to 640,000
