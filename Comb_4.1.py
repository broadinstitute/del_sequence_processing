...
SelectionFile = "Shoc2_ConstructC_Alpha.csv"
LibraryFile = "Shoc2_Original_Lib_Alpha.csv"
...
with open(SelectionFile[:len(SelectionFile)-4]+"_subfamilies.csv", 'w') as f:
...
with open(SelectionFile[:len(SelectionFile)-4]+"_enrichments.csv", 'w') as z:
    for x in range (1,len(selc)):
        ye = ''
        l = l+1
        if float(libc[x])==0:
